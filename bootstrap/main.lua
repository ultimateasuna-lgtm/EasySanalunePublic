...
SlashCmdList["EASYSANALUNE"] = function(msg)
	...
	raw = string.gsub(raw, "%s+$", "")
	local cmd, args = string.match(raw, "^(%S+)%s*(.-)$")
	...
	elseif cmd == "debug" then
		if UI and UI.SetCritDebugFromSlash then
			UI.SetCritDebugFromSlash(args)
		else
			L_print_text("[debug crit] indisponible")
		end
	elseif cmd == "syncdebug" then
		if UI and UI.SetMobSyncDebugFromSlash then
			UI.SetMobSyncDebugFromSlash(args)
		else
			L_print_text("[debug sync] indisponible")
		end
	else
