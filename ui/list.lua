...
local function L_get(key, ...)
  local locale = rawget(_G, "EasySanaluneLocale")
...
local function is_network_offensive_rand_entry(entry)
  return infer_rand_role_from_entry(entry) == "offensive"
end
...
UI.make_scroll_elem = function(parent, charlink)
  ...
  local function run_local_rand(min, max, skipChatAnnounce)
    if (not skipChatAnnounce) and STATE and STATE.raid_announce then
      local channel = nil
      if IsInRaid() then
        channel = "RAID"
      ...
      if channel then
        local announceText = L_get("ui_rand_announce", escape_rand_announce_text(get_display_name()))
        ---@diagnostic disable-next-line:deprecated
        SendChatMessage(announceText, channel)
      end
    end
    if UI and UI.MarkCurrentPlayerTurnPlayed then
  ...
  secure:SetScript("PreClick", function(_, button)
    ...
    local canSendToMJ = is_network_offensive_rand_entry(obj.infos)
    ...
    if UI.SendRandRequest then
      requestId = UI.SendRandRequest(min, max, get_display_name(), displayInfo, obj.infos and obj.infos.rand_role)
    end
    if requestId then
      run_local_rand(min, max)
    end
