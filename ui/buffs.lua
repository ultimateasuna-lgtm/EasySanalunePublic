...
local function collect_bonus_details(randName, randRole)
  ...
  if normalizedRole == "" and normalizedRandName ~= "" and type(STATE.CHARS) == "table" then
    for i = 1, #STATE.CHARS do
      local entry = STATE.CHARS[i]
      if type(entry) == "table" and normalize_name(entry.name) == normalizedRandName then
        normalizedRole = normalize_name(entry.rand_role)
        break
      end
    end
  end
